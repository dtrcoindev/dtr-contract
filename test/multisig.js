...
const DtrCoin = artifacts.require('DtrCoin.sol');
const DtrCoinMultiSigFactory = artifacts.require('DtrCoinMultiSigWallet.sol');
const web3 = DtrCoin.web3;
const eth = web3.eth;
...
const deployMultiSigWallet = (owners, requiredConfirm, coin) => {
  return DtrCoinMultiSigFactory.new(owners, requiredConfirm, coin, "vault");
};

const deployDtrCoin = (owner,admin,vault) => {
  return DtrCoin.new("Dtr", "DtrCoin", owner, admin, vault);
};
...
contract("DtrCoinMultiSigWallet", (accounts) => {
  const BIG = (v) => new theBN.BN(v)
  let coinInstance;
  let walletInstance;
  const requiredConfirmations = 2;

  let OneDtrCoinInMinunit;
  ...
  beforeEach(async () => {
    coinInstance = await deployDtrCoin(owner, admin, vault);
    assert.ok(coinInstance);

    NoOfTokensInMinunit = await coinInstance.totalSupply();
    OneDtrCoinInMinunit = await coinInstance.getOneDtrCoin();
  ...
  it("multisig wallet basic functions", async () => {
    assert.equal( await walletInstance.dtrCoin(), coinInstance.address );
  ...
  it("only signers can request TX", async () => {
    // user1 cannot
    assertRevert("1-1",() => walletInstance.requestTransferInDtr(user1, 1, 0, "test 1", {from: user1}));
    ...
    // owner cannot
    assertRevert("2-1",() => walletInstance.requestTransferInDtr(user1, 1, 0, "test 2-1", {from: owner}));
    ...
    // admin(as signer) can
    assertNotRevert("3-1",() => walletInstance.requestTransferInDtr(user1, 1, 0, "test 3-1", {from: admin}));
    ...
    // vault(as signer) can
    assertNotRevert("4-1",() => walletInstance.requestTransferInDtr(user1, 1, 0, "test 4-1", {from: vault}));
    ...
    assertNotRevert("5-1",() => walletInstance.requestTransferInDtr(user1, 1, 0, "test 5-1", {from: signer}));
  ...
  it("signers only methods", async () => {
    // request a TX
    walletInstance.requestTransferInDtr(user1, 1, 0, "test 1", {from: signer});
  ...
  it("test transfer using multisig wallet", async () => {
    ...
    const thousandDtrInMinunit = OneDtrCoinInMinunit.mul(BIG(1000))
    coinInstance.transfer(walletInstance.address, thousandDtrInMinunit, {from:vault});
    assert.equal(await balanceOf(walletInstance.address), thousandDtrInMinunit.toString());

    // Add dtrcoin transfer transaction
    // example ABI generation:
    // const transferData = coinInstance.contract.transfer.getData(user1, OneDtrCoin * 1);
    //

    const MSG1 = "send 1Dtr and 1DENNIS to user1";
    const trSubmit = await walletInstance.requestTransferInDtr(user1, 1, 1, MSG1, {from:vault});
    ...
    assert.equal(afterBalance.toString(), OneDtrCoinInMinunit.mul(BIG(1)).add(BIG(1)).toString());
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestTransferInDtr(user2, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestTransferInDtr(user1, 1, 0, "test addsigner-1", {from: user1}));
