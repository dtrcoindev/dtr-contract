...
/**
 * DtrCoin contract learning test
 *   deploy in before, and no deploy between it()
 */
contract("DtrCoin", (accounts) => {
  ...
  let NoOfTokensInMinunit, OneDtrCoinInMinunit, NoOfTokensInDtr;
  let coin
  ...
  before(async () => {
    ...
    OneDtrCoinInMinunit = BIG(await coin.getOneDtrCoin());
    NoOfTokensInDtr = NoOfTokensInMinunit.div(OneDtrCoinInMinunit)
  });
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.symbol(), "DTR");
    assert.equal(await coin.name(), "DTRCoin");
    assert.equal(await coin.decimals(), 9);
    assert.equal((await coin.totalSupply()).toString(), BIG(NOOFTOKENS).mul(OneDtrCoinInMinunit).toString());
  ...
  it("token distribution", async () => {
    ...
    await coin.transfer(admin, OneDtrCoinInMinunit, {from: vault});
    assert.equal(await balanceOf(admin), OneDtrCoinInMinunit.toString());

    // transfer 0.5 token from admin to owner
    let halfDtr = OneDtrCoinInMinunit.div(BIG(2))
    await coin.transfer(owner, halfDtr, {from: admin});
    assert.equal(await balanceOf(owner), halfDtr.toString());
    assert.equal(await balanceOf(admin), halfDtr.toString());
    assert.equal(await balanceOf(vault), (NoOfTokensInDtr.sub(BIG(1)).mul(OneDtrCoinInMinunit).toString()));
  });
  ...
  it("marginal transfers", async () => {
    ...
    try {
      await coin.transfer(user2, OneDtrCoinInMinunit.mul(BIG(1)), {from: user1});
      assert.fail();
    }
    ...
    try
    {
      await coin.transfer(user2, OneDtrCoinInMinunit.mul(BIG(0)), {from: user1});
      assert.equal(await balanceOf(user1), OneDtrCoinInMinunit.mul(BIG(0)).toString());
      assert.equal(await balanceOf(user2), OneDtrCoinInMinunit.mul(BIG(0)).toString());
    }
    catch(exception) { assert.fail(); }

    // transfer 0 token from vault(balance > 0) to user1
    try {
        await coin.transfer(user1, OneDtrCoinInMinunit.mul(BIG(0)), {from: vault});
        assert.equal(await balanceOf(user1), OneDtrCoinInMinunit.mul(BIG(0)).toString());
    } catch(exception) { assert.fail(); }

    // transfer whole balance to other
    await coin.transfer(user1, OneDtrCoinInMinunit.mul(BIG(1)), {from: vault});
    assert.equal(await balanceOf(user1), OneDtrCoinInMinunit.mul(BIG(1)).toString());
    await coin.transfer(user2, OneDtrCoinInMinunit.mul(BIG(1)), {from: user1});
    assert.equal(await balanceOf(user1), OneDtrCoinInMinunit.mul(BIG(0)).toString());
    assert.equal(await balanceOf(user2), OneDtrCoinInMinunit.mul(BIG(1)).toString());
  });
  ...
  it("only admin can set reserve", async () => {
    await coin.setReserve(user1, OneDtrCoinInMinunit, {from: admin});
  ...
  it("reserve and reserveOf", async () => {
    ...
    await coin.transfer(user1, OneDtrCoinInMinunit.mul(BIG(10)), { from: vault });
    await coin.setReserve(user1, OneDtrCoinInMinunit.mul(BIG(5)), { from: admin });
    assert.equal(await reserveOf(user1), OneDtrCoinInMinunit.mul(BIG(5)).toString());
  });
  
  it("transfer after setting reservation", async () => {
    // user1 = 10 Dtr
    await coin.transfer(user1, OneDtrCoinInMinunit.mul(BIG(10)), { from: vault });
    // user1 = 10 Dtr with 5 Dtr reserve
    await coin.setReserve(user1, OneDtrCoinInMinunit.mul(BIG(5)), { from: admin });
    // user1 = 9 Dtr with 5 Dtr reserve
    await coin.transfer(user2, OneDtrCoinInMinunit.mul(BIG(1)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneDtrCoinInMinunit.mul(BIG(9)).toString());

    // user1 = 6 Dtr with 5 Dtr reserve
    coin.transfer(user2, OneDtrCoinInMinunit.mul(BIG(3)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneDtrCoinInMinunit.mul(BIG(6)));

    try {
      let tx = await coin.transfer(user2, OneDtrCoinInMinunit.mul(BIG(2)), {from: user1}); // violating reserved amount
      assert.equal(await balanceOf(user1), OneDtrCoinInMinunit.mul(BIG(6)));
      assert.fail();
    }
    catch(exception) {
