 ...
 * @title coin contract
 */
contract DtrCoin is ERC20Capped, ERC20Detailed, ERC20Burnable {
...
// File: contracts/DtrCoinSafeMath.sol
...
// File: contracts/DtrCoinMultiSigWallet.sol
...
/**
 * @title DtrCoin Multi Signature contract
 */
contract DtrCoinMultiSigWallet {
    /*  Enums  */
    ...
    /* Contract Created */
    event MultiSigCreated(address indexed creator, address indexed DtrCoin);
    ...
    using DtrCoinSafeMath for uint8;
    using SafeMath for uint256;
     ...
     * @param _DtrCoinAddress DtrCoin address
     *
     */
    constructor (address[] memory _signers, uint8 _requiredSign, address _DtrCoinAddress, string memory _description) public {
        ...
        nextTx = 0;
        dtrCoin = DtrCoin(_DtrCoinAddress);
        owner = msg.sender;
        description = _description;
        emit MultiSigCreated(owner, _DtrCoinAddress);
    }
    ...
    /**
      ...
      * @param _amountUnit amount of DtrCoin(Play part) to be transferred
      * @param _amountMinunit amount of DtrCoin(Minunit part) to be transferred
      */
    ...
    // DtrCoin.transfer will be fail with reason out of gas. Then the TX will remain
    ...
    function runTX(uint16 txid) internal senderIsSigner returns(bool) {
        ...
        if(request.txType == TxType.TRANSFER) {
            // transfer money
            dtrCoin.transfer(request.who, request.amount);
            ...
            dtrCoin.setAdmin(request.who);
        } else if(request.txType == TxType.OWNER_CHANGE) {
            dtrCoin.setOwner(request.who);
        } else if(request.txType == TxType.VAULT_CHANGE) {
            dtrCoin.setVault(request.who);
        } else {
            return false;
        }
        return true;
